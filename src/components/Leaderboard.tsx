...
export default function Leaderboard({
  ...
  const loadScores = () => {
    try {
      ...
      if (stored) {
        ...
        safeStorage.setItem("neon_merge_scores", JSON.stringify(defaults));
        setScores(defaults);
      }
    } catch (e) {
  ...
  const handleSaveScore = (e: React.FormEvent) => {
    ...
    try {
      ...
      safeStorage.setItem("neon_merge_scores", JSON.stringify(trimmedScores));
  ...
  const handleClearScores = () => {
    if (window.confirm(language === "en" ? "Clear all scores?" : "सभी स्कोर हटाएं?")) {
      safeStorage.removeItem("neon_merge_scores");
      setScores([]);
    }
  };
