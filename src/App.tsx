...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      const storedScores = safeStorage.getItem("neon_merge_scores");
      if (storedScores) {
    ...
    try {
      const storedAch = safeStorage.getItem("neon_merge_achievements");
      if (storedAch) {
  ...
  const handleScoreSaved = () => {
    ...
    try {
      const stored = safeStorage.getItem("neon_merge_scores");
      if (stored) {
  ...
  const triggerAchievementUnlock = (id: string) => {
    setAchievements((prev) => {
      ...
      safeStorage.setItem("neon_merge_achievements", JSON.stringify(updated));
      return updated;
    });
  };
